...
(function(GLOBAL) {
	...
	function _recursivelyFind(element, property, expression, index) {
		element = $prototype(element), expression = expression || 0,
				index = index || 0;
	...
	function up(element, expression, index) {
		...
		if (arguments.length === 1)
			return $prototype(element.parentNode);
...
(function() {
					...
					Hash,
					{
						...
						_begin : function() {
							...
							if (width && (positionedWidth === width)) {
								...
								var parent = element.parentNode, pLayout = $prototype(
										parent).getLayout();
	...
	function getOffsetParent(element) {
		...
		if (!isInline && element.offsetParent)
			return $prototype(element.offsetParent);

		while ((element = element.parentNode) && element !== document.body) {
			if (Element.getStyle(element, 'position') !== 'static') {
				return isHtml(element) ? $prototype(document.body)
						: $prototype(element);
			}
		}
...
Form.Methods = {
	...
	findFirstElement : function(form) {
		var elements = $prototype(form).getElements().findAll(
				function(element) {
...
if (!document.getElementsByClassName)
	document.getElementsByClassName = function(instanceMethods) {
		...
		return function(className, parentElement) {
			return $prototype(parentElement || document.body)
